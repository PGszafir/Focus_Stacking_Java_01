...
public class FotoDecomposition {
    ...
    public FotoDecomposition(String nazwaObrazu) {
        ...
        pixelArrayARGB = new int[image.getHeight()][image.getWidth()][4];
        for (int i = 0; i < image.getWidth() * image.getHeight(); i++) {
            int x = i / image.getWidth(); //pozycja na osi poziomej
            int y = i % image.getWidth(); //pozycja na osi pionowej
            pixelArrayARGB[x][y][0] = (pixelArray[i] >> 24) & 0xFF; //a, alfa
            pixelArrayARGB[x][y][1] = (pixelArray[i] >> 16) & 0xFF; //r, czerwony
            pixelArrayARGB[x][y][2] = (pixelArray[i] >> 8) & 0xFF; //g, zielony
            pixelArrayARGB[x][y][3] = (pixelArray[i]) & 0xFF; //b, niebieski
        }
    }
}
